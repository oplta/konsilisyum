...
class KeyPool:
    ...
    def mask_secrets(self, text: str) -> str:
        ...
        # Sort keys by length descending to ensure longer keys are masked before their substrings
        sorted_keys = sorted(
            [k.key for k in self.keys.values() if k.key],
            key=len,
            reverse=True
        )
        ...
        for raw_key in sorted_keys:
            if raw_key in masked_text:
                if len(raw_key) > 8:
                    masked = f"{raw_key[:4]}...{raw_key[-4:]}"
                else:
                    masked = "***"
                masked_text = masked_text.replace(raw_key, masked)
    ...
    def report_error(self, key_id: str, error: str, retry_after: int | None = None):
        ...
        # Sanitize error: mask any managed keys that appear in the error message
        sanitized_error = self.mask_secrets(error)

        key.last_error = sanitized_error
        key.error_count += 1
