...
class TestMessage:
    def test_create_message(self):
        m = Message(
            turn=1, speaker="Atlas", content="Merhaba", speaker_type=SpeakerType.AGENT, topic="Test"
        )
        assert m.turn == 1
...
class TestMemoryManager:
    def test_add_and_retrieve_messages(self):
        mem = MemoryManager()
        m1 = Message(
            turn=1, speaker="Atlas", content="Bir", speaker_type=SpeakerType.AGENT, topic=""
        )
        m2 = Message(
            turn=2, speaker="Mira", content="Iki", speaker_type=SpeakerType.AGENT, topic=""
        )
        mem.add_message(m1)
        mem.add_message(m2)
...
class TestKeyPool:
    ...
    def test_fallback_to_pool(self):
        ...
        agent = Agent(name="Mira", role="R", goal="G", blind_spot="B", style="S", trigger="T")
        key = pool.get_key(agent)
        assert key.id == "k2"
